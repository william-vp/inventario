    ...
    <table id="tablaCreditos" class="table table-hover" cellspacing="0" width="100%">
        ...
        @foreach($creditos as $credito)
            <tr>
                <td>{{$credito->id}}</td>
                <?php
                $fecha_credito= gmmktime(
                    date_format(date_create($credito->fecha), 'g'),
                    date_format(date_create($credito->fecha), 'i'),
                    date_format(date_create($credito->fecha), 's'),
                    date_format(date_create($credito->fecha), 'm'),
                    date_format(date_create($credito->fecha), 'd'),
                    date_format(date_create($credito->fecha), 'Y'));
                $date= explode(' ',$credito->fecha);
                ?>
                <td><?php
                    setlocale(LC_TIME, 'es_ES');
                    $fecha= strftime("%A, %d de %B de %Y", $fecha_credito);
                    echo ucfirst($fecha);
                    echo "<br><strong>Hora: </strong>".$date[1];
                    ?></td>
